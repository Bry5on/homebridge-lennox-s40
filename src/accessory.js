...
class LennoxZoneAccessory {
  constructor(platform, zoneId, displayName, existingAccessory) {
    ...
    this._writer = new CoalescedSetpointWriter(
      (m, ...a) => this.log(`[${this.displayName}] ${m}`, ...a),
      async ({ hspF, cspF }) => {
        await this.platform.setZoneSetpointsViaSchedule(this.zoneId, {
          hsp: Math.round(hspF),
          csp: Math.round(cspF),
        });
      },
  ...
  applyZoneStatus(status) {
    ...
    const ambientF =
      typeof status.temperature === "number"
        ...
        : typeof status.temperatureC === "number"
          ? Math.round(this.cToF(status.temperatureC))
          : undefined;
