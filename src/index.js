...
const LENNOX_HVAC = new Set(["off", "heat", "cool", "heat and cool", "emergency heat"]);
...
class LennoxS40Platform {
  constructor(log, config, api) {
    ...
    if (!this.host) { this.log.error("[LennoxS40] No host configured."); return; }
    this.client = new LccClient({
      host: this.host, clientId: this.clientId, verifyTLS: this.verifyTLS,
      longPollSeconds: this.longPollSeconds, logBodies: this.logBodies, log: (m) => this.log.debug(m),
    });
    ...
    api.on("didFinishLaunching", async () => {
      try {
        if (this.resetOnBoot && this.cachedByUUID.size > 0) {
          ...
          try { this.api.unregisterPlatformAccessories(PLUGIN_NAME, PLATFORM_NAME, stale); } catch (e) {
            this.log.warn(`[LennoxS40] prune error: ${e.message}`);
          }
          this.cachedByUUID.clear(); this.cachedByZoneId.clear();
          await new Promise((r) => setTimeout(r, 200));
        }
        await this.sessionStart();
        for (const zoneId of this.zoneIds) {
          if (!this.zoneAccessories.has(zoneId)) {
            this.zoneAccessories.set(zoneId, new LennoxZoneAccessory(this, zoneId, this.cachedByZoneId.get(zoneId) || null));
          }
        }
        this.startPump();
      } catch (e) {
  ...
  manualScheduleId(zoneId) { return 16 + Number(zoneId); }

  async setZoneHvacMode(zoneId, lennoxMode) {
    if (!LENNOX_HVAC.has(lennoxMode)) throw new Error(`unsupported HVAC mode: ${lennoxMode}`);
    const sid = this.manualScheduleId(zoneId);
    this.log(`[LennoxS40] zone=${zoneId} systemMode -> ${lennoxMode} scheduleId=${sid}`);
    await this.client.setZoneSchedule(zoneId, sid);
    await new Promise((r) => setTimeout(r, 150));
    await this.client.setSchedulePeriod(sid, 0, { systemMode: lennoxMode });
  ...
  async setZoneSetpointsViaSchedule(zoneId, { hsp, csp }) {
    ...
    try {
      await this.client.setZoneConfigScheduleHold(zoneId, {
        enabled: true, exceptionType: "hold", scheduleId: sid, expirationMode: "nextPeriod", expiresOn: "0",
      });
      holdArmed = true;
    } catch (e) { this.log.warn(`[LennoxS40] scheduleHold failed: ${e.message}`); }
    if (!holdArmed) {
      try {
        await this.client.setScheduleHold(zoneId, sid, { hsp: period.hsp, csp: period.csp, type: "temporary", expirationMode: "nextPeriod" });
        holdArmed = true;
      } catch (e) { this.log.warn(`[LennoxS40] setScheduleHold failed: ${e.message}`); }
    }
    try { await this.client.requestData(["/zones"]); } catch {}
  }
  ...
  applyMessages(msgs) {
    for (const m of msgs) {
      ...
      for (const z of data.zones) {
        ...
        if (schedHold && typeof schedHold.scheduleId === "number") this.holdScheduleId.set(zoneId, schedHold.scheduleId);
    ...
    for (;;) {
      try {
        const { messages, nextStartTime } = await this.client.retrieve({
          startTime: this._retrieveStartTime, count: 60, timeoutSec: this.longPollSeconds,
        });
        backoff = 2;
        if (messages.length) {
        ...
        try { await this.sessionStart(); this.log("[LennoxS40] Session re-established; RequestData re-issued."); }
        catch (re) { this.log.warn(`[LennoxS40] Reconnect failed: ${re.message}`); }
...
module.exports = (api) => { api.registerPlatform(PLATFORM_NAME, LennoxS40Platform); };
